...
/**
@brief 幾何補正用にマスクに直方形におさまる画像を作成する。
@details 幾何補正時に通常通り生画像を使うと投影領域をはみ出た部分が見えなくなってしまうため、<br>
中心にリサイズした黒縁の画像を作成する。
@param src 投影に使用する生画像
@param dst 幾何補正用画像
*/
void SQUARE::adaptImageShape(cv::Mat &src, cv::Mat &dst)
{
	setMask(RECTMASK_ADDRESS);
	loadData();
	// すべての画像をプロジェクタ解像度へ
	cv::resize(src, src, cv::Size(projWidth, projHeight));
	cv::Point center = this->resize(src, dst, cv::Point(this->Xa, this->Ya), cv::Point(this->Xb, this->Yb));
	imageShift(dst, dst, center);
	cv::resize(dst, dst, cv::Size(camWidth, camHeight));
}
		...
		case 'S':
			{
				...
				saveImage(src, RECTMASK_ADDRESS);
...
void SQUARE::drawResult(bool savePoint){
	...
	if(savePoint){
		...
		cv::imwrite(RECTMASK_DEBUG_ADDRESS, kakunin);
		std::ofstream ofs(RECTVALUE_ADDRESS);
		ofs << Xa << "\t" << Ya << std::endl;
		ofs << Xb << "\t" << Yb << std::endl;
		ofs.close();
	}
...
{
	std::ifstream ifs("./Data/Rect_value.txt");
...
void SQUARE::onMouse( int event, int x, int y, int flag, void* param)
{
	...
	case cv::EVENT_LBUTTONUP:
		...
		// 結果を保存
		std::ofstream ofs("./Data/Rect_value.txt");
		ofs << Xa << "\t" << Ya << std::endl;
		ofs << Xb << "\t" << Yb << std::endl;
		ofs.close();
		break;
